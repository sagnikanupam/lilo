...
def _evalHelper(s):
    ...
    elif (isNum(eqTree.left.root) and isNum(eqTree.right.root)):
...
def _divselfHelper(s):
    eqTree = treefy(s)
    if (eqTree.root == "/" and eqTree.left == eqTree.right and eqTree.left.root != "None"):
        return detreefy(Tree(1))
    return s
...
def _subsubHelper(s):
    eqTree = treefy(s)
    if eqTree.root == "-" and isNum(eqTree.right.root) and eqTree.right.root < 0:
...
def _zerodivHelper(s):
    eqTree = treefy(s)
    if eqTree.root == "/" and eqTree.right.root == 0:
        return detreefy(Tree(0))
    return s
...
def mathPrimitives(LARGEST_CONSTANT: int = 10):
    ...
    return [
        ...
        Primitive("conpole_subself", arrow(tstr, tint, tstr), _subsub),
