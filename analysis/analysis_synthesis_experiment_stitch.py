...
analyzer = SynthesisExperimentAnalyzer(experiment_name=EXPERIMENT_NAME, experiment_types=["dreamcoder"], allow_incomplete_results=True, domains = ["re2", "clevr"], compute_likelihoods=COMPUTE_LIKELIHOODS, batch_size = 96, seeds = [0],)
df_runtime = analyzer.get_runtime_metrics()
print(df_runtime)
print(df_runtime.groupby(["domain", "experiment_type", "model_type", "model_fn", "task_split"]).time_elapsed.agg("mean"))
bp = sns.barplot(data=df_runtime.query("model_fn == 'infer_programs_for_tasks'"), y="experiment_type", x="time_elapsed", hue="task_split")
bp.savefig('re2_bar_plot.png')
